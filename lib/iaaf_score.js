...
/*
 Regular expressions to match jumps and throws events.
 Everything else is a track event.
 */
const IAAF_JUMP_CODES = new RegExp("^(LJ|PV|TJ|HJ)$", "i");
const IAAF_THROW_CODES = new RegExp("^(WT|JT|DT|HT|SP)$", "i");
...
var iaafScoringObjects = {};

// Utility function to get the <gender>-<event> scoring key
function iaafScoringKey(gender, event_code) {
    var key = (gender + "-" + event_code);
    return key.toUpperCase();
}

/*
 Function to handle lazy evaluation of _iaaf_scoring_objects, which maps from
 scoring key to parameters
 */
function iaaFScoringObjectsCreate() {
        var n = IAAF_SCORING_TABLE.length;
        for (var i = 0; i < n; i++) {
            var o = IAAF_SCORING_TABLE[i];
            iaafScoringObjects[iaafScoringKey(o.gender, o.event_code)] = o;
        }
    }

/*
 Function to determine IAAF score, based on gender, event and performance.
 Note that value is <seconds> for track events, <metres> for throws events and
 <centimetres> for jumps events.
 */
var iaaf_score = function (gender, event_code, value) {
    // Create _iaaf_scoring_objects (lazily evaluated)
    iaaFScoringObjectsCreate();

    // Inner function (closure) to get the score
    function fNew(gender, event_code, value) {
        var coeffs =
            iaafScoringObjects[iaafScoringKey(gender, event_code)];
        ...
        if (event_code.match(IAAF_JUMP_CODES)) {
            return Math.floor(coeffs.A * Math.pow(value - coeffs.Z, coeffs.X));
        } else if (event_code.match(IAAF_THROW_CODES)) {
            return Math.floor(coeffs.A * Math.pow(value - coeffs.Z, coeffs.X));
        } else {
...
/*
 Function to determine performance required to achieve IAAF score, given gender
 and event
 Note that performance is <seconds> for track events, <metres> for throws events
 and <centimetres> for jumps events.
 */
exports.iaaf_performance = function iaaf_performance(gender, event_code, score) {
    // Create _iaaf_scoring_objects (lazily evaluated)
    iaaFScoringObjectsCreate();

    var coeffs = iaafScoringObjects[iaafScoringKey(gender, event_code)];
    ...
    // Handle based on whether jumps, throws or track event
    if (event_code.match(IAAF_JUMP_CODES)) {
        return Math.ceil((Math.pow(score / coeffs.A, 1.0 / coeffs.X)) +
            coeffs.Z);
    } else if (event_code.match(IAAF_THROW_CODES)) {
        return Math.ceil(100.0 *
                ((Math.pow(score / coeffs.A, 1.0 / coeffs.X)) + coeffs.Z)) /
            100.0;
    } else {
        return Math.floor(100.0 *
                (coeffs.Z - (Math.pow(score / coeffs.A, 1.0 / coeffs.X)))) /
            100.0;
    }
};